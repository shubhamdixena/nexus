...
CREATE INDEX IF NOT EXISTS idx_correction_history_correction ON public.data_correction_history(correction_id);
CREATE INDEX IF NOT EXISTS idx_correction_history_performer ON public.data_correction_history(performed_by);
CREATE INDEX IF NOT EXISTS idx_correction_history_date ON public.data_correction_history(created_at);
...
BEGIN
    ...
    IF OLD.status IS DISTINCT FROM NEW.status THEN
        INSERT INTO public.data_correction_history (
            correction_id, action, performed_by, old_status, new_status, notes
        ) VALUES (
            NEW.id,
            CASE 
                ...
                WHEN NEW.status = 'implemented' THEN 'implemented'
                WHEN NEW.status = 'pending' AND OLD.status != 'pending' THEN 'reopened'
                ELSE 'reviewed'
            END,
...
CREATE TRIGGER trigger_log_correction_status_change
    AFTER UPDATE ON public.data_correction_reports
    FOR EACH ROW
