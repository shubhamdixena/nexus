...
# Create new dataframe with rounds replacing the Application Deadlines column
columns = df.columns.tolist()
deadline_col_index = columns.index('Application Deadlines')

# Build new column list
new_columns = columns[:deadline_col_index] + ['R1', 'R2', 'R3', 'R4', 'R5'] + columns[deadline_col_index+1:]

# Create new dataframe with the correct structure
new_df = pd.DataFrame()

# Add columns before deadline columns
for col in columns[:deadline_col_index]:
    new_df[col] = df[col].values

# Add round columns
for col in ['R1', 'R2', 'R3', 'R4', 'R5']:
    new_df[col] = rounds_df[col].values

# Add columns after deadline columns
for col in columns[deadline_col_index+1:]:
    new_df[col] = df[col].values
...
print(f"Replaced Application Deadlines column with: {', '.join(round_columns)}")
